...
| `agentic_search` | Perform multi-turn retrieval with a configurable LLM backend. The agent rewrites the query, issues tool calls against the configured corpus and containers, and returns the most relevant documents. See [docs/AGENTIC-SEARCH.md](docs/AGENTIC-SEARCH.md) for setup and configuration. |
