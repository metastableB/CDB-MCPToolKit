...
| `agentic_search` | Perform multi-turn retrieval with a configurable LLM backend. The agent rewrites the query, issues tool calls against the configured corpus and containers, and returns the most relevant documents. Requires a separate retrieval service, where the agent, Cosmos access, and models are configured. This tool calls that service over HTTP. See [docs/AGENTIC-SEARCH.md](docs/AGENTIC-SEARCH.md) for setup and configuration. |
