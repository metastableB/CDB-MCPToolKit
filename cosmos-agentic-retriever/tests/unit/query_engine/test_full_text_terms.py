...
def test_tokenize_lowercases() -> None:
    assert tokenize_for_fts("Hello WORLD FooBar") == [
        "hello",
        "world",
        "foobar",
    ]
...
def test_tokenize_splits_on_punctuation() -> None:
    assert tokenize_for_fts("foo, bar. baz! qux?") == [
        "foo",
        "bar",
        "baz",
        "qux",
...
def test_removed_term_limit_is_rejected() -> None:
    with pytest.raises(TypeError, match="max_terms"):
        tokenize_for_fts("battery recycling", max_terms=1)
